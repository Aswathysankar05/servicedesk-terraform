...
const SupportViewTickets: React.FC = () => {
        ...
        const fetchTickets = async () => {
            ...
            try {
                ...
                if (type === 'incidents') {
                    ...
                    fetchedTickets = [
                        ...incidents.filter((ticket: Incident) => ticket.statusInfo.status_id !== '2' && ticket.statusInfo.status_id !== null),
                        ...requests.filter((ticket: Request) => ticket.statusInfo.status_id !== '2' && ticket.statusInfo.status_id !== null),
                        ...changes.filter((ticket: Change) => ticket.statusInfo.status_id !== '2' && ticket.statusInfo.status_id !== null)
                    ];
                    ...
                    fetchedTickets = [
                        ...incidents.filter((ticket: Incident) => ticket.inc_priority === 'P1'),
                        ...requests.filter((ticket: Request) => ticket.req_priority === 'P1'),
                        ...changes.filter((ticket: Change) => ticket.chng_priority === 'P1')
                    ];
                }
                ...
                if (sortedTickets.length > 0) {
                    ...
                    setTickets([]); // Set to empty array if no tickets are found
                }
                // setTickets(sortedTickets);
                setLoading(false);
            } catch (err) {
    ...
    // alert("currentTickets"+currentTickets.length)
    ...
    const handleSupportDashboard = () => {
        //setActiveComponent('support-dashboard');
    ...
    const handleIncCreate = () => {
        navigate(`/support-dashboard/supportviewtickets/createticket/${type}`);
    };
    ...
    <div className="user-dashboard">
        ...
        <div className="ticket-list">
            {/* <h5>All Tickets</h5> */}
            {tickets.length === 0 ? (
                    <p>No data available for your tickets.</p> // Show message if no tickets
                ) : (
            <table aria-label="Ticket List">
                ...
                <tbody>
                    {currentTickets.map(ticket => {
                        ...
                        return (
                            <tr key={ticketId} 
                            // onClick={() => navigate(`/support-dashboard/supportviewtickets/supportworkhistory/${type}/${workid}`)}
                            > 
                                <td>
                                <a
                                href="#"
                                onClick={(e) => {
                                    e.preventDefault(); 
                                    navigate(`/support-dashboard/supportviewtickets/supportworkhistory/${type}/${workid}`);
                                }}
...
export default SupportViewTickets;
