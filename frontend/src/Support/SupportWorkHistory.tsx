...
const ViewWorkHistory: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
	...
	try {
		...
		const latestDataResponse = await fetch(`${BASE_URL}/${type}/${workid}`);
		...
		// Check the latest closed_at value
		const workHistoryTimestamp =latestData.updated_at ? new Date(latestData.updated_at).toISOString() : null;
		if(type === "incidents"){
		...
		const newWorkHistory = {
			...
			performed_by: Loginemail, 
			timestamp: workHistoryTimestamp, 
		};
  ...
  // Handles the comment submit form
	...
	// 	sendLogToBackend('Error in updating: ' + error, 'error');
	// 	  }
    ...
    const handleProfile = () => {
        ...
        navigate('/supportdashboard/support-profile');
    };
    const handleRefresh = () => {
        window.location.reload();
    };
